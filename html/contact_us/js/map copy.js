...
function processMap() {
  ...
  // to hide the distributor info when clicking outside of the map

  jQuery(document).ready(function () {
    var $distributorInfo = jQuery("#distributer-info");

    // Show distributor info when clicking a country
    jQuery(document).on("click", ".jvectormap-container path", function () {
        $distributorInfo.show(); // Ensure it is shown when a country is clicked
    });

    // Click event for hiding distributor info when clicking outside
    jQuery(document).click(function (event) {
        if (
            !$distributorInfo.is(event.target) && // Click is NOT on distributor info
            $distributorInfo.has(event.target).length === 0 && // Click is NOT inside distributor info
            !jQuery(event.target).closest(".jvectormap-container").length // Click is NOT on the map
        ) {
            if ($distributorInfo.children().length > 0) {
                $distributorInfo.empty().hide(); // Hide only if populated
            }
        }
    });

    // Ensure distributor info is visible when populated (even after hiding)
    ...
    // Attach this function after populating distributor info
    jQuery(document).on("DOMSubtreeModified", "#distributer-info", showDistributorInfo);
});
}